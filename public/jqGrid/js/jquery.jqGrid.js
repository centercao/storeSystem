		...
		this.createTableForGroup = function(group, parentgroup) {
			...
			inputAddRule.on('click',function() {
				...
				group.rules.push({
					field: cm.name,
					op: opr[0],
					data: cm.searchoptions.defaultValue||"" /* 第二次添加默认值 */
				}); // adding a new rule
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				if( (!defaultFilters && colnm) || p.multipleSearch === false  ) {
					...
					if(cmi >=0 && columns[cmi].searchoptions && columns[cmi].searchoptions.sopt) {
						...
						data = columns[cmi].searchoptions.defaultValue||"";
					} else if(p.sopt && p.sopt.length) {
						cmop = p.sopt[0];
						data = p.defaultValue||"";
					}
					defaultFilters = {groupOp: "AND", rules: [{field: colnm, op: cmop, data: data}]};
				}
				found = false;
...
$.jgrid.extend({
	editGridRow : function(rowid, p){
		...
		p = $.extend(true, {
			top : 0,
			left: 0,
			width: 'auto',/* 500 add edit form 宽度*/
			datawidth: 'auto',
			height: 'auto',
			dataheight: 'auto',
			modal: false,
		...
		return this.each(function(){
			...
			dw = isNaN(rp_ge[$(this)[0].p.id].datawidth) ? rp_ge[$(this)[0].p.id].datawidth : rp_ge[$(this)[0].p.id].datawidth+"px",//style='width:"+dw+";height:"+dh+
			frm = $("<form name='FormPost' id='"+frmgr+"' class='FormGrid' onSubmit='return false;'></form>").data("disabled",false),
			tbl;
			if(templ) {
		...
		return this.each(function() {
			...
			if ($("#"+alertIDs.themodal)[0] === undefined) {
				if(!o.alerttop && !o.alertleft) {
					if (window.innerWidth !== undefined  && window.innerWidth !=0) {  //&& window.innerWidth !=0
				...
				if($t.p._nvtd) {
					if(o.dropmenu) {
						...
						if($t.p.responsive) {
							...
							$(pgid+"_"+o.position,pgid).append(navtbl);//.width(twd);/* 修正导航栏三部分不居中*/
						}
						$t.p._nvtd[0] = twd;
					} else {
